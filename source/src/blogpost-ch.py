...
fg = FeedGenerator()
fg.id("http://nicdemon.github.io")
fg.title("Nicolas de Montigny's Portfolio")
fg.author({"name": "Nicolas de Montigny", "email": "nicolas.de.montigny0323@gmail.com"})
fg.link(href="http://nicdemon.github.io", rel="alternate")
fg.logo("http://nicdemon.github.io/build/_static/img/profile-circle.png")
fg.subtitle("Nicolas de Montigny's Portfolio!")
fg.link(href="http://nicdemon.github.io/rss.xml", rel="self")
fg.language("en")

# Add all my posts to it
for ix, irow in posts.iterrows():
    fe = fg.add_entry()
    fe.id(f"http://nicdemon.github.io/{irow['path']}")
    fe.published(irow["date"])
    fe.title(irow["title"])
    fe.link(href=f"http://nicdemon.github.io/{irow['path']}")
...
def run_directive(name, data):
    ...
    category = str(opts.get("category"))
    tag = str(opts.get("tag"))
...
if __name__ == "__main__":
    ...
    if args.directive:
        ...
        declare_result(run_directive(args.directive, data), sys.stdout)
    elif args.transform:
